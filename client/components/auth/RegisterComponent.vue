<template>
    ...
    <AuthFormComponent @authEmit="handleSubmit">
        ...
        <div class="mt-5">
            <label for="email" class="text-white text-xs tracking-widest font-light">Confirm your password</label>

            <div>
                <input class="w-72 h-7 mt-1 rounded" type="password" v-model="passwordConfirmed" required>
            </div>
        </div>
...
const handleSubmit = async () => {
    ...
    const trimmedPasswordConfirmed = passwordConfirmed.value.trim();

    // check if empty values, if yes display errors
    if (!trimmedEmail || !trimmedPassword || !trimmedPasswordConfirmed) {
        showLoading.value = false;
        displayError.value = true;
        password.value = '';
        passwordConfirmed.value = '';
        displayErrorMessage.value = 'Invalid credentials provided';
        return;
    }

    // make post request to api for register
    const request = await fetch('http://localhost:5000/api/v1/auth/register', {
        ...
        body: JSON.stringify({
            ...
            'password_confirmation': trimmedPasswordConfirmed
        }),
    });
    ...
    if (response.errors) {
        // destruct response
        const {errors: {email, password}} = response;
        ...
        if (email) {
            ...
            password.value = '';
            passwordConfirmed.value = '';
            displayErrorMessage.value = email.shift().replace(/\.$/, '');
            return;
        } else if (password) {
            showLoading.value = false;
            displayError.value = true;
            password.value = '';
            passwordConfirmed.value = '';
            displayErrorMessage.value = password.shift().replace(/\.$/, '');
            return;
        } else {
            showLoading.value = false;
            displayError.value = true;
            password.value = '';
            passwordConfirmed.value = '';
            displayErrorMessage.value = 'Internal error. Try again later'
            return;
        }
    }
